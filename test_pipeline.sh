...
echo "  Entry point: python -m pipeline.evaluator --config params.yaml"
echo "  Input:  *_CCF_RVs.csv + params.yaml (pipeline_io section)"
...
echo "  Entry point: python -m mcmc.batch --config params.yaml"
echo "  Input:  *_CCF_RVs.csv + lmfit_summary.csv + params.yaml (mcmc_params section)"
...
echo "  For a quick test, reduce steps in params.yaml:"
...
echo "  Stage 2 (Fit):       python -m pipeline.evaluator    --config params.yaml"
echo "  Stage 3 (Select):    streamlit run mcmc/selector_app.py"
echo "  Stage 4 (MCMC):      python -m mcmc.batch            --config params.yaml"
echo ""
